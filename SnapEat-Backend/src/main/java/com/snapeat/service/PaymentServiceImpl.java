...
public class PaymentServiceImpl implements PaymentService {
    ...
    public void verifyCheckoutSession(String sessionId) {
        ...
        try {
            ...
            boolean wasAlreadyPaid = order.getPaymentStatus() == PaymentStatus.PAID;
    ...
    public void handleWebhook(String payload, String sigHeader) {
        ...
        switch (event.getType()) {
            ...
            case "checkout.session.completed" -> {
                ...
                markOrderPaidFromCheckoutSession(session);
            }
    ...
    private Session extractCheckoutSession(Event event) {
        return event.getDataObjectDeserializer()
                .getObject()
                .map(obj -> (Session) obj)
                .orElseThrow(() -> new PaymentException("Could not deserialize Checkout Session from event"));
    }

    private void markOrderPaidFromCheckoutSession(Session session) {
        String orderIdText = session.getMetadata() != null
                ? session.getMetadata().get("orderId")
                : null;
        if (orderIdText == null || orderIdText.isBlank()) {
        ...
        if (orderIdText == null || orderIdText.isBlank()) {
            log.warn("checkout.session.completed missing order mapping: sessionId={}", session.getId());
            return;
        }

        Long orderId = Long.valueOf(orderIdText);
        orderRepository.findByIdWithUser(orderId).ifPresentOrElse(order -> {
            boolean wasAlreadyPaid = order.getPaymentStatus() == PaymentStatus.PAID;
            order.setPaymentStatus(PaymentStatus.PAID);
            if (session.getPaymentIntent() != null) {
                order.setStripePaymentIntentId(session.getPaymentIntent());
            ...
            log.info("Order {} marked paid from checkout.session.completed (sessionId={})", orderId, session.getId());
        }, () -> log.warn("No order found for checkout session mapping: orderId={}, sessionId={}", orderId, session.getId()));
    }
